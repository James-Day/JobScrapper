...
class JobScrapper(private var current:Context)  {


     private fun setNewText(input: String, layout: LinearLayout) {

          //  val gigsLayout = findViewById<LinearLayout>(R.id.gigsLayout)
          val newGig = TextView(current)
          newGig.text = input
          newGig.textSize = 12F
          layout.addView(newGig)
     }

     private suspend fun setTextOnMainRoutine(input: String, layout: LinearLayout) {
          withContext(Dispatchers.Main) {
               setNewText(input, layout)
          }
     }
val list = arrayListOf<String>(

)
     //List of all craigslist websites that will be searched
     ...
     private var sitesDone = 0
     private fun createURL(sitesDone: Int, todayOnly: Boolean): String {
          val todayOnlyString: String = if (todayOnly) {
               "&postedToday=1"
          } else ""
          val searchString = "/search/cpg?is_paid=yes$todayOnlyString"
          return sites[sitesDone] + searchString
     }
     ...
     suspend fun getRequest(
          ...
          var sitesFound = 0
          while(sitesFound != numSites) {
               val url = URL(createURL(sitesDone, todayOnly))
               setTextOnMainRoutine("trying: $url", layout)

               val connection = url.openConnection() as HttpURLConnection
               val input = connection.inputStream
               val noLocal = Regex("Zero local results found")
               val nothingFound = Regex("Nothing found for that search")
               ...
               BufferedReader(InputStreamReader(input)).use {
                    ...
                    if (response.contains(noLocal)) {
                         setTextOnMainRoutine("skipping because Zero local results found \n", layout)
                    } else if (response.contains(nothingFound)) {
                         setTextOnMainRoutine("Skipping because nothing found for that search \n", layout)
                    } else {
                         var hasGigs = false //this boolean is used to determine if any gigs were found
                         response.delete(0,response.length)
                         val connection = url.openConnection() as HttpURLConnection
                         val inputStream = connection.inputStream

                         val banNearby = Regex("ban nearby")     //you've searched too far
                         val resultRow = Regex("result-row")
                         val endList = Regex("</div>") //if you find the final div tag, then you're at the end of the list
                         val resultDate = Regex("result-date")
                         val resultTitle = Regex("result-title")

                         BufferedReader(InputStreamReader(inputStream)).use {
                              var line = it.readLine()
                              while(line != null){
                                   if(line.contains(banNearby)){ //searched too far
                                        break
                                   }
                                   if(line.contains(resultRow)){
                                        line = it.readLine()
                                        while(!line.contains(endList)){
                                             if(numDays > 0 && line.contains(resultDate)){

                                                  var date = line.substringAfter("datetime=").substringBefore("title")
                                                  date = date.substring(1,11) // parses the string for the date of the post

                                                  val localDateVar = LocalDate.parse(date)
                                                  val dateVar = Date.from(localDateVar.atStartOfDay(
                                                       ZoneId.systemDefault()).toInstant())
                                                  val calendar = Calendar.getInstance()        //if the date is before
                                                  calendar.add(Calendar.DATE, numDays * -1)    //the check after date
                                                  val checkAfterDate = calendar.time           //then stop checking dates
                                                  if(dateVar < checkAfterDate){                //on that page.
                                                       break
                                                  }
                                             }

                                             if(line.contains(resultTitle)){
                                                  val gigUrl = line.substringAfter("href=").substringBefore("data-id")
                                                  val gigTitle = line.substringAfter(">").substringBefore("</a>")
                                                  if(!hasGigs){ //if this website hasn't added gigs to the list yet, then add the url
                                                       list.add("\n\n"+ gigUrl)
                                                  }
                                                  list.add("*** $gigTitle")
                                                  hasGigs = true
                              ...
                              if(hasGigs) {
                                   sitesFound++
                                   setTextOnMainRoutine("found \n", layout)
                              }
                              else{
                                   setTextOnMainRoutine("skipping because no gigs in date range \n", layout)
                              }
                         }
                    }
               sitesDone++
